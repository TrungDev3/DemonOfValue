...
public class BattleGameplayManager : MonoBehaviour
{
    ...
    {
        // Lặp qua tất cả đơn vị anh hùng còn sống
        for (int i = 0; i < aliveHeroUnits.Count; i++)
        {
            // Gán tất cả đơn vị địch còn sống làm mục tiêu cho đơn vị anh hùng hiện tại
            aliveHeroUnits[i].AssignTargetUnits(aliveEnemyUnits);
        }

        // Lặp qua tất cả đơn vị địch còn sống
        for (int i = 0; i < aliveEnemyUnits.Count; i++)
        {
            // Gán tất cả đơn vị anh hùng còn sống làm mục tiêu cho đơn vị địch hiện tại
